...
namespace THA_WEEK4_MEGAN_R
{
    public partial class SecondWindowsForm : Form
    {
        ...
        private void butt_MAGIC_Click(object sender, EventArgs e)
        {
            ...
            if (magic == false)
            {
                MessageBox.Show("You have to choose one background color :(", "Oh no!", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            if (magicc == false)
            {
                MessageBox.Show("You have to choose one text color :(", "Oh no!", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            form1.Show();
        }
    }
